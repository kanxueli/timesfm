...
ft_backbone_save_checkpoint_dir=./checkpoints/ft_backbone
 
rm predictions_and_trues.json
rm wandb -rf
# rm $ft_backbone_save_checkpoint_dir -rf
...
CUDA_VISIBLE_DEVICES=4 python3 finetune.py \
 ...
 --num-features 1 \
 --dataset-type "IOH" \
 --checkpoint-dir="./checkpoints/ft_backbone" \
 --num-epochs=100 \
 --learning-rate=1e-1 \
 --adam-epsilon=1e-3 \
...
#     --checkpoint-path=$ft_backbone_save_checkpoint_dir \
#     --data-path=$dataset_path \
#     --horizon-len=150 \
#     --context-len=450 \
#     --batch-size=1 \
